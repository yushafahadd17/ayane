...
const serialize = async (conn, m, store) => {
  ...
  conn.getFile = async (PATH, returnAsFilename) => {
        ...
        if (data && returnAsFilename && !filename)(filename = path.join(__dirname, '../database/tmp/' + new Date * 1 + '.' + type.ext), await fs.promises.writeFile(filename, data))
        return {
            res,
            filename,
            ...type,
  ...
  conn.downloadMedia = async (message, pathFile) => {
  ...
  conn.sendImageAsStickerV2 = async (jid, path, quoted, options = {}) => {
    let buff = Buffer.isBuffer(path)
      ? path
      : /^data:.*?\/.*?;base64,/i.test(path)
      ? Buffer.from(path.split`,`[1], "base64")
